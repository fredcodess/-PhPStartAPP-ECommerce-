...
Class databaseTable{
    ...
    public function __construct($table,$primaryKey){
        $this->pdo = new PDO('mysql:host=mysql;dbname=GroupWork;charset=utf8','student','student');
        $this->table = $table;
    ...
    public function findAllProductsSearch($searchValue){
        $value = ['value'=>'%'.$searchValue.'%'];
        $stmt=$this->pdo->prepare('SELECT * FROM '.$this->table.' WHERE (p_name LIKE :value OR description LIKE :value)');
        $stmt->execute($value);
        return $stmt->fetchAll();
    }
    ...
    public function findRecordsByMultipleFieldsOrdered($record, $orderfield, $order){
        $keys = array_keys($record);
        $values = implode(', ', $keys);
        $valuesWithColon = implode(', :', $keys);
        $stmt=$this->pdo->prepare('SELECT * FROM '.$this->table.' WHERE ('.$values.')=(:'.$valuesWithColon.') ORDER BY '.$orderfield.' '.$order);
        $stmt->execute($values);
        return $stmt->fetchAll();
    }
...
$productsTable = new databaseTable('products','id');
$questionsTable = new databaseTable('questions','question_id');
$answersTable = new databaseTable('answers','answer_id');
$categoriesTable = new databaseTable('categories','category_id');
